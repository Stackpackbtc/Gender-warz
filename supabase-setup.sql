...
alter table public.profiles
  ...
  add column if not exists verification_badge text not null default null;

-- Keep the admin identity tied to the authenticated email, not to a browser-only check.
...
-- Automatically create a profile when a new member signs up.
...
begin
  insert into public.profiles (
    id, username, display_name, side, is_admin, verified, verification_status, verification_badge
  ) values (
    new.id,
...
create trigger on_auth_user_created_gender_warz
after insert on auth.users
for each row execute function public.handle_new_gender_warz_user();
...
set is_admin = true,
    ...
    display_name = coalesce(nullif(display_name,''), 'Stack Pack')
where lower(p.id::text) in (select lower(id::text) from auth.users where lower(email) = lower('Stackpackmedia@gmail.com'));
...
-- Members can read their own profile and edit their own non-admin fields.
...
-- Admins can see/manage all profiles. The admin function is SECURITY DEFINER to avoid recursive RLS.
...
-- Verification applications: members submit/read their own; admin controls all.
...
-- Public can see only the fields needed for a verified badge.
...
-- Ensure the admin identity is always official.
...
begin
  if exists (select 1 from auth.users u where u.id = new.id and lower(u.email) = lower('Stackpackmedia@gmail.com')) then
    ...
    new.verification_badge := 'official';
...
create trigger keep_stackpack_official_trigger
before insert or update on public.profiles
for each row execute function public.keep_stackpack_official();
